...
public interface OutboundMessageJpaRepository extends JpaRepository<OutboundMessage, Long> {
     ...
     * Не захватывает строки — см. {@link #claimPending} для реального single-fire.
     */
    @Query("SELECT m FROM OutboundMessage m WHERE m.status = 'PENDING' AND m.createdAt <= :now "
    ...
    /**
     * P2-3: базовый повтор без backoff (полноценный backoff/circuit breaker — P2-6) —
     * пока {@code attempts + 1 < maxAttempts}, запись возвращается в {@code PENDING} и
     * будет подхвачена следующим тиком поллера; иначе переходит в терминальный
     * {@code FAILED} (задел под DLQ P2-6 — ручной reprocess там же переведёт обратно
     * в PENDING).
     */
    @Modifying(clearAutomatically = true)
