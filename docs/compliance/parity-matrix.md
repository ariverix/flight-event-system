...
| 15.3 | Горизонтальное масштабирование (HPA) | ЧАСТИЧНО | Архитектура stateless + leader election готова к HPA. Helm-чарты и HPA-конфигурация — задача P8-1 | ADR-0004 |
...
| R-3 | 15.3 HPA (Kubernetes) | Helm-чарты и HPA не готовы | Задача P8-1; не блокирует функциональную приёмку, но требуется до прод-развёртывания |
| R-4 | ГОСТ TLS | Не реализован на уровне приложения (терминация на proxy) | Путь задокументирован (`docs/security/gost-tls-path.md`); решение до прод-развёртывания — задача инфраструктуры заказчика |

---

*Матрица актуализируется при каждом изменении кодовой базы, влияющем на паритет. Последнее обновление: P8-3, ветка main (коммит 5a48130+).*
